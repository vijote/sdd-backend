...
compatibility: "Requires spec-kit project structure with .coda/ directory"
metadata:
...
- Check if `.coda/extensions.yml` exists in the project root.
...
- If no hooks are registered or `.coda/extensions.yml` does not exist, skip silently

## Outline

You are updating the project constitution at `.coda/memory/constitution.md`. The active
...
1. Run `.coda/scripts/powershell/resolve-template.ps1 constitution-template -Json` from the repository root and parse `TEMPLATE_CONTENT` as the active template.
   ...
   - If it fails, stop and report the resolution error; do not continue with only one contributing
     template layer.
   - If `.coda/memory/constitution.md` exists, load it as the source of current project-specific
...
6. Write the completed constitution back to `.coda/memory/constitution.md` (overwrite).
...
Write only `.coda/memory/constitution.md`; do not create or modify template source files.
